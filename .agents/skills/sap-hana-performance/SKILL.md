...
Use `hdbsql` for direct HANA performance diagnostics. See [hdbsql-diagnostic-queries.md](references/hdbsql-diagnostic-queries.md) for ready-to-run queries covering expensive statements, execution plans, and column-store memory consumption.

> **Cross-reference**: For a full catalog of CLIs available in the Devin sandbox, see skill `sap-cli-toolbelt`.

## Worked example

**Scenario**: Report `ZFI_ORDER_HISTORY` takes 40 minutes on HANA (was 8 minutes on Oracle). SQLM shows 14,200 individual `BSEG` reads — a classic N+1 pattern compounded by `SELECT *` on a 300-column compatibility view.

See [worked-example-order-history.md](references/worked-example-order-history.md) for the full diagnosis (SQLM, ATC, SRTCM) and CDS view refactoring with before/after code.

**Key outcome**: Runtime drops from **40 minutes to 12 seconds** after replacing N+1 `BSEG` reads with a single CDS view entity joining `ACDOCA` to `KNA1`.
