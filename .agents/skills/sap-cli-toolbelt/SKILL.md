...
**Scenario**: A Devin session runs S/4HANA readiness checks against package `$Z_SALES` on an on-prem ECC system, downloads failing objects, cleans them with abap-cleaner, pushes them back, and verifies with unit tests — all without SAP GUI or Eclipse.

See [worked-example-atc-clean-pipeline.md](references/worked-example-atc-clean-pipeline.md) for the full 4-step pipeline: install + authenticate → ATC readiness check → download/clean/push → ABAP Unit verification.

**Key outcome**: End-to-end `sapcli atc` → `abap-cleaner` → `sapcli aunit` pipeline completes from the command line.
