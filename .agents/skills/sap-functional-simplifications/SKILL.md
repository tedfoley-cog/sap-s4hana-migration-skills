...
**Scenario**: A custom pricing report `Z_PRICE_LIST` uses hard-coded `TYPE c LENGTH 18` for material numbers and is flagged by ATC `S4HANA_READINESS` / `MATNR_LENGTH` check.

See [worked-example-matnr-price-list.md](references/worked-example-matnr-price-list.md) for the full 7-step walkthrough covering code fix, DDIC structure update, ALV adjustment, and downstream interface flagging.

**Key outcome**: All hard-coded `c(18)` material number fields replaced with `TYPE matnr` (40 chars), preventing silent truncation of longer material numbers.
