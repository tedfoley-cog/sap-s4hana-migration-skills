...
**Scenario**: Running the `S4HANA_READINESS_2025_FPS01` check variant against class `ZCL_CUSTOMER_HELPER` on a central ATC system. The check produces 7 findings spanning MATNR length, Business Partner migration, and obsolete ALV types.

See [worked-example-zcl-customer-helper.md](references/worked-example-zcl-customer-helper.md) for the full 6-step walkthrough covering variant verification, SDB refresh, check execution, triage table, ADT remediation, and delta run.

**Key outcome**: 5 of 7 findings resolved via Quick Fixes. 2 remaining findings (BP migration) require manual rewrite tracked separately.
