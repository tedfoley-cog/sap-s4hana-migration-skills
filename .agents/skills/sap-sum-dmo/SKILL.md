...
**Scenario**: Convert ECC 6.07 (EHP7) system `E01` on Oracle 19c to S/4HANA 2025 FPS01 on HANA. Source database: 4.2 TB. Target downtime window: 16 hours.

See [worked-example-sum-dmo-conversion.md](references/worked-example-sum-dmo-conversion.md) for the full 5-step walkthrough covering Maintenance Planner stack generation, pre-checks, DMO flavor selection, SUM execution with phase timings, and post-conversion verification.

**Key outcome**: Standard DMO completes within 16-hour window using 16 parallel R3load jobs and table splitting. Data archiving reduces source from 4.2 TB to 3.1 TB.
