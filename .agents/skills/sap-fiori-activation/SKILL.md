...
**Scenario**: Activate the **Manage Sales Orders** app (App ID `F0095`) for sales representatives after a system conversion to S/4HANA 2023, including Spaces and Pages configuration.

See [worked-example-manage-sales-orders.md](references/worked-example-manage-sales-orders.md) for the full 7-step walkthrough covering app lookup, task list execution, OData/ICF verification, PFCG role setup, Space/Page assignment, and cache invalidation.

**Key outcome**: `F0095` tile appears in the "Sales" Space on the Fiori Launchpad. Sales orders display correctly with org-level authorizations applied.
