...
**Scenario**: Acme Retail performs OTC regression testing across an ECC 6.0 EHP8 → S/4HANA 2024 brownfield conversion. 12 test scripts cover 4 OTC process variants impacted by Business Partner, Output Management, MATDOC, and credit management simplifications.

See [worked-example-otc-regression.md](references/worked-example-otc-regression.md) for the full walkthrough covering process variant identification, 12 test scripts in Solution Manager, sandbox refresh, SUM dry run with 5 failure root-cause analyses, fix/re-run cycle, and UAT scheduling.

**Key outcome**: Dry run #1 finds 5 failures (CVI sync, Output Management, KONV compatibility view, CVI customizing, ACDOCA BAdI). All fixed; dry run #2 passes 12/12 tests.
