...
**Scenario**: A sales order credit approval workflow must block orders exceeding the customer's available credit before save. The decision tree leads to Tier 3 (Developer Extensibility) using a released BAdI.

See [worked-example-credit-approval.md](references/worked-example-credit-approval.md) for the full decision tree walkthrough, ABAP for Cloud Development implementation sketch, and a counter-example showing when the same requirement would use Tier 2 (Side-by-Side on BTP) instead.

**Key outcome**: Synchronous validation via released BAdI `BADI_SD_SLS_DOC_VALIDATION` (C0 contract) and CDS views `I_SalesDocument` / `I_CreditAccountManagement` (C1 contracts). Clean core compliant.
