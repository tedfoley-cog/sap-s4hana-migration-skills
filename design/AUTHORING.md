...
Do NOT edit anything under `plugins/`. That directory is regenerated by `scripts/sync-to-plugins.sh`.
...
- ❌ Add Claude Code-only fields like `commands`, `agents`, `hooks`, or `mcp`. They will be stripped by the sync script and waste reviewer time.
- ❌ Edit sibling skills, even if you think they're wrong. File an issue or PR comment instead.
- ❌ Edit `plugins/` by hand. Run the sync script.
...
PRs that don't include the regenerated `plugins/` mirror will fail CI.
